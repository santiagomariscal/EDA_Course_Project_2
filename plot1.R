...
# Prepare data for chart of total emissions by year by gruping by year and calculating total emissions per year
plot_data <- summarySCC_PM25 %>% 
        group_by(year) %>%
        summarise(total_pollutant = sum(Emissions))

# Create and export bar chart of total pollutant emission by year
