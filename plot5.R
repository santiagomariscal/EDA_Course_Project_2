...
# Summarize total emissions per year for motor vehicle sources in Baltimore City
plot_data <- summarySCC_PM25 %>%
        filter(SCC %in% sources_data$SCC, fips == "24510") %>%
        group_by(year) %>%
        summarise(total_pollutant = sum(Emissions))

# Create and export bar chart of motor vehicle pollutant emission by year for Baltimore
...
ggplot(data = plot_data, aes(x = year, y = total_pollutant)) + 
        ...
        ggtitle("Motor vehicle related PM2.5 emissions in Baltimore City") +
